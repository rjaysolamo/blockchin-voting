...
CREATE POLICY IF NOT EXISTS "Authenticated users can view onchain entity mapping"
ON public.onchain_entity_map FOR SELECT
TO authenticated
USING (true);

-- Only admins can manage mappings directly.
CREATE POLICY IF NOT EXISTS "Admins can manage onchain entity mapping"
...
CREATE POLICY IF NOT EXISTS "Users can view own onchain student registry"
ON public.onchain_student_registry FOR SELECT
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Admins can manage onchain student registry"
...
CREATE POLICY IF NOT EXISTS "Users can view own onchain whitelist"
ON public.onchain_voter_whitelist FOR SELECT
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Admins can manage onchain whitelist"
